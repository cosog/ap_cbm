Ext.define('AP.store.DailyEvaluation.CBMWellDailyAnalysisStatStore', {
	extend : 'Ext.data.Store',
	autoLoad : true,
	pageSize : defaultPageSize,
	proxy : {
		type : 'ajax',
		url : context + '/dailyEvaluationController/statisticsData',
		actionMethods : {
			read : 'POST'
		},
		params : {
	...
	listeners : {
		load:function(store,record,f,op,o){
			initCBMWellDailyStatPieChat(store);
			
			var gridPanel = Ext.getCmp("CBMWellAnalysisDaily_Id");
            if (isNotVal(gridPanel)) {
            	gridPanel.getStore().load();
            }else{
            	Ext.create('AP.store.DailyEvaluation.CBMWellDailyAnalysisiListStore');
            }
		},
